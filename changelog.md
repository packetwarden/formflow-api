...
### Added
- **Runner Password + Turnstile Protected Forms v1**: Added password-protected public form unlock flow (`POST /api/v1/f/:formId/access`), HMAC-signed form access tokens, server-side Cloudflare Turnstile verification on unlock + submit, builder access settings route (`PATCH /api/v1/build/:workspaceId/forms/:formId/access`), Worker-safe bcrypt password hashing, default-on Turnstile rollout for active forms, and least-privilege revocation of direct authenticated `password_hash` selects.
- **Cascade Soft Delete Security-Definer Fix v1**: Added `project-info-docs/migrations/2026-03-07_cascade_soft_delete_security_definer_v1.sql` and updated canonical `formflow_beta_schema_v2.sql` so `public.cascade_soft_delete()` runs as `SECURITY DEFINER`, restoring builder soft-delete after least-privilege table grants prevented trigger-side updates to `form_submissions`.
- **Server-Side Validation Hardening v1**: Added shared request-validation primitives in `src/utils/validation.ts`, extracted runner-compatible form contract parsing into `src/utils/form-contract.ts`, enforced runner-compatible schema checks on build create/save/publish paths, fail-closed runner protection for `require_auth` / password / captcha-gated forms, and hardened Stripe internal-auth + webhook request validation.
- **Runner Service-Role Secret-Key DB Compatibility v1**: Added `project-info-docs/migrations/2026-03-07_runner_service_role_secret_key_compat_v1.sql` and updated canonical `formflow_beta_schema_v2.sql` so `submit_form(...)` authorizes trusted backend execution via PostgREST request role with legacy JWT-role fallback, restoring hosted `sb_secret_*` compatibility without widening grants.
- **Build Submission Read Grants v1**: Added `project-info-docs/migrations/2026-03-07_build_submission_read_grants_v1.sql` and updated canonical `formflow_beta_schema_v2.sql` to grant authenticated least-privilege `SELECT` access to the admin-safe `form_submissions` columns required by build submission review endpoints, while keeping `idempotency_key` and `encrypted_pii` unreadable.
- **Runner Hosted Secret-Key Compatibility Hardening**: Updated `src/db/supabase.ts` to support hosted Supabase secret/publishable keys (`sb_secret_*`, `sb_publishable_*`) without forwarding them as invalid `Authorization: Bearer <key>` headers, preserving backend-only secret usage while keeping legacy JWT `service_role` compatibility.
- **Runner Backend Misconfiguration Signal**: Updated `src/routes/f/index.ts`, `src/types/index.ts`, and runner docs/tests so trusted submit RPC auth/config failures now return `500` with code `RUNNER_BACKEND_AUTH_MISCONFIGURED` instead of a misleading public `403`.
- **Builder Submission Read API**: Added `GET /api/v1/build/:workspaceId/forms/:formId/submissions` and `GET /api/v1/build/:workspaceId/forms/:formId/submissions/:submissionId` in `src/routes/build/index.ts` with authenticated workspace-scoped reads, stable keyset pagination on (`created_at`, `id`), and response filtering that excludes `idempotency_key` and `encrypted_pii`.
- **Canonical V2 Launch Baseline Sync**: Updated `project-info-docs/formflow_beta_schema_v2.sql` to include all launch-required DDL for fresh installs through the March 6, 2026 contract, including Stripe customer mapping, durable checkout idempotency, webhook claim leasing, plan-cache sync/reconciliation, and Stripe snapshot apply RPCs. Fresh environments no longer require the post-V2 migration chain to satisfy the current backend contract; existing migrated environments are unchanged.
- **Function Search-Path Hardening v3**: Added `project-info-docs/migrations/2026-03-06_function_search_path_hardening_v3.sql` and pinned `SET search_path = ''` on `public.set_updated_at()` and `public.cascade_soft_delete()` to resolve Security Advisor "Function Search Path Mutable" findings.
- **Least-Privilege Grants Hardening v1**: Added `project-info-docs/migrations/2026-03-05_grants_least_privilege_hardening_v1.sql`, revoked broad table grants from `authenticated`, removed direct `anon` table access on core builder tables (`profiles`, `workspaces`, `workspace_members`, `forms`, `form_versions`, `form_submissions`), and restricted direct authenticated writes to explicit `public.forms` column-level `INSERT`/`UPDATE` grants. This removes direct hard-delete grant paths on soft-delete tables (`profiles`, `workspaces`, `forms`, `form_submissions`) while preserving existing Worker route behavior.
...
- **SECURITY DEFINER Hardening v2**: Added `project-info-docs/migrations/2026-02-27_security_definer_hardening_v2.sql` to pin `search_path` on remaining SECURITY DEFINER trigger functions, enforce `publish_form(...)` actor binding (`p_published_by = auth.uid()`), enforce service-role runtime guard in `submit_form(...)`, tighten function execute grants (`check_request` anon-only, explicit grants for `get_workspace_entitlements` and `get_published_form`), and revoke `CREATE` on schema `public` from `PUBLIC`.
- **Runner Submission Strict Gateway Hardening v1**: Added `project-info-docs/migrations/2026-02-27_runner_submission_gateway_hardening_v1.sql`, removed direct `anon/authenticated` insert path to `form_submissions`, restricted `submit_form(...)` execute to `service_role`, and updated runner submit route to call `submit_form` via trusted service-role client.
